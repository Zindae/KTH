...
#
.equ    timer1count,49999999    # Change this value (and this comment!)
...
main:
    ...
    movia	r6, 0x800020		# se
    movia 	r7, stub			# ja
    ldw		r8, 0(r7)			# lägger in första ordet i stub till addressen som ligger i r6
    stw		r8, 0(r6)			# detta görs för samtliga 3 ord som rutinen stub består av
    ldw		r8, 4(r7)
    stw		r8, 4(r6)
    ldw		r8, 8(r7)
    stw		r8, 8(r6)
    ...
    movi 	r5, timer1_period_low 		# address till timer1_low
	movi 	r6, timer1_period_high		# address tll timer_high
	movia	r8, timer1count
	stwio 	r8, 0(r5)
	srai	r8, r8, 16
	stwio	r8, 0(r6)
	movi	r5, timer1_control
	movi 	r6, 0x07					# 0x07 means 111, and will turn on the timer, with continious mode and each 
	stwio	r6, 0(r5)					# cycle will trigger an interupt (since index 2 = 1)
...
exchand:
    ...
    rdctl	r5, estatus
    andi	r5, r5, 1
    beq		r5, r0, not_interupt
    
    # Then we should check if ipending is nonzero
    # (Condition 2).
    rdctl	r6, ipending
    beq		r6, r0, not_interupt
...
not_interupt:
    ...
    movi	r6, 0x3a			
	subi	r5, ea, 4				# i en Trap instr. så har bit 0-5 bitmasken 0x3a 
	ldw		r8, 0(r5)				# och bit 11-16 bitmask 0x2d
	andi	r7, r8, 0x3f			# vi kollar värdet på *(ea - 4) och läser in det i r8
	bne		r7, r6, not_trap		# kolla om den första bitmasken stämde
	
	movia	r6, 0x2d
	slli	r6, r6, 11 	
	and		r7, r8, r6	
	bne		r7, r6, not_trap		# å den andra
	br		exc_was_trap			# det stämde, aka vi hade en trap som instruktion.
...
keyint:
	...
	movia 	et, key4_ack	#defined somewhere higher up
    ldwio	r10, 0(et)
    
    
    stwio   r0,0(et)    	# ack the interrupt
	...
	movia	r5,	keys4_base	
	ldwio	r6, 0(r5)		
	andi	r6, r6, 4			#only check key2
	PUSHMOST					#Gonna call a subroutine
	beq		r6, r0, key2was_up		#is down now, was up
...
key0int:
	...
	movia	r5,	keys4_base	
	ldwio	r6, 0(r5)		
	andi	r6, r6, 1			#only check key0
	PUSHMOST					#Gonna call a subroutine
	beq		r6, r0, was_up		#is down now, was up
