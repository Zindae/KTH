 ...
 */
#define TIMER_1_TIMEOUT (49999999)
...
void irq_handler_toggles(void* , alt_u32);
...
void toggles_init(void) {
    int check_value;
    /*activate interrupts from toggle0 and toggle1*/
    *de2_pio_toggles18_intmask=1;
    /*set edgecap to zero*/
    *de2_pio_toggles18_edgecap=0;
    /*register the interrupter handler*/
    check_value=alt_irq_register(de2_pio_toggles18_intindex, NULL_POINTER, irq_handler_toggles);
    
    /*if the shit hits the fan*/
    if (check_value) n2_fatal_error();
}
...
void irq_handler_toggles(void* context, alt_u32 irqnum) {
    ...
    /*address of red leds*/
    volatile int* red_led=(volatile int*) 0x810;
    ...
    if (*de2_pio_toggles18_edgecap & 0x1) {
        *de2_pio_toggles18_edgecap=0;
        /*light a red led, put an 'S', wait a while, put an 's', turn off red led*/
        *red_led=1;
        out_char_uart_0('S');
        save_value = alt_irq_interruptible( irqnum +1 );
        bigdelay();
...
void irq_handler_uart(void* context, alt_u32 irqnum) {
    volatile int * uart_base = UART_0;
    char c = (char) *uart_base & 0xFF;
    out_char_uart_0(c);
}
...
void uart_init(void) {
    volatile int * uart_base = UART_0;
    int check_value;
    *(uart_base + 3) = *(uart_base + 3) | 0x80;
    /*register the interrupter handler*/
    check_value=alt_irq_register(4, NULL_POINTER, irq_handler_uart);
