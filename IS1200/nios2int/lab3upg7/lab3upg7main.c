...
{ 
    ...
    /*if an edge has been captured on redled0...*/
    if (*de2_pio_toggles18_edgecap & 0x1) {
    *de2_pio_redled18_base = 0x1;
    out_char_uart_0('S'); /*light a red led, put an 'S', wait a while, put an 's', turn off red led*/
    
    save_value = alt_irq_interruptible (irqnum );
    bigdelay();
    ...
    *de2_pio_redled18_base = 0;
     ...
     * de2_pio_toggles18_edgecap = 0;                /* cleara edgecapture, kvittera och acknowledge interrupt*/
}
   /* Initialize uart_0 for interrupts. */
void toggles_int( void )
{ 
  /* Declare a temporary for checking return values
   * from system-calls and library functions. */
   int checkvalue;

  /* Disable interrupts for uart_0, if they were enabled.
   * The function alt_irq_disable returns an int, which
   * always has the value 0. The type cast to void tells the
   * compiler that we really want to ignore the value. */

  (void) alt_irq_disable( de2_pio_toggles18_intindex ); /* Disable interrupts for toggles_18 if they were enabled.*/
  
  /* Allow interrupts from RXRDY only. */
  *de2_pio_toggles18_intmask = 1; /*aktivera avbrott från toggles 0 */  

  /* Set up Altera's interrupt wrapper for interrupts 
   * from uart_0. The function alt_irq_register will 
   * enable interrupts from uart_0.
   * Return value is zero for success,
   * nonzero for failure. */

  checkvalue = alt_irq_register( de2_pio_toggles18_intindex, NULL_POINTER, irq_handler_toggles);

  /* If there was an error, terminate the program. */               
  if( checkvalue != 0 ) n2_fatal_error();
